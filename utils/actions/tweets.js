...
import { getObjectValue, setObjectValue } from "../storageFunctions";
...
export const get_feed_tweets = async (maxID = null) => {
	let feedTweetsArr;
	...
	if (maxID === null) {
		...
		feedTweetsArr = await getObjectValue(collationNames.TWEETS + tweetsKeys.FEED_TWEETS);
		feedTweetsArr != null ? feedTweetsArr : [];
	}
	try {
		...
		if (response.status == 200) {
			...
			await setObjectValue(collationNames.TWEETS + tweetsKeys.FEED_TWEETS, feedTweetsArr);
...
export const get_who_to_follow = async (username) => {
	...
	try {
		...
		if (response.status == 200) {
			...
			await setObjectValue(collationNames.TWEETS + tweetsKeys.WHO_TO_FOLLOW, userWhoToFollowArr);
		} else if (response.status == 401 || response.status == 428) {
			// Unauthorized
...
export const get_notifications = async () => {
	...
	try {
		...
		if (response.status == 200) {
			...
			await setObjectValue(collationNames.TWEETS + tweetsKeys.NOTIFICATIONS, userNotificationsArr);
		} else if (response.status == 401 || response.status == 428) {
			// Unauthorized
...
export const get_tweet_screen = async (tweetData) => {
