...
export default class Tweet extends React.Component {
	constructor(props) {
		...
		const twit = "Let's get it! 💙\n\nYou can follow today's game live on CITY+ with a subscription - https://t.co/BfVGtMgtQW https://t.co/YM571qZL8k";
		this.state = {
			photo: { uri: "https://pbs.twimg.com/profile_images/1244262677163257857/tXYybiqv_normal.jpg" },
			touched: false,
			tweet: twit,
			...
			retweetedBy: ["Sandra", "Hannit", "Michael", "Jason", "Queen"][
				Math.floor(Math.random() * ["Sandra", "Hannit", "Michael", "Jason", "Queen"].length)
			],
		};
		this.tweetPressed = this.tweetPressed.bind(this);
	...
	retweet() {
		...
		if (retweeted) this.setState({ retweeted: false, retweets: retweets - 1 });
		else this.setState({ retweeted: true, retweets: retweets + 1 });
	}
	like() {
		const { liked, likes } = this.state;

		if (liked) this.setState({ liked: false, likes: likes - 1 });
